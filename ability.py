...
class Ability(object):
    ...
    def is_legal(self, user):
        if (user.is_dead or user.BTH < self.BTHcost):
            return False
...
class Breath(Ability):
    def __init__(self):
        ...
        self.description = ["Devastating fire breath:BTH {}".format(self.BTHcost)]
        self.type = "special"
        self.level = 3
    ...
    def on_start(self, user, targets):
        ...
        for target in targets:
            power = max(0, int(round(power - target.RESmod)))
            if user.roll() + user.DEXmod + int(user.level) > 10:
...
class Dash(Ability):
    ...
    def sentence_on_use(self, seconds, power):
        return "Dashes {}: {:+d} DEX".format(seconds, power)
...
class Frenzy(Ability):
    ...
    def sentence_on_use(self, seconds, power):
        return "Frenzy {}: {:+d} STR".format(seconds, power)      
...
class Poison(Ability):
    def __init__(self):
        ...
        self.is_passive = 0
        self.recoil = "MED_RECOIL"
        self.description = ["Poison attack"]
        self.type = "offense"
        self.poisoned = []
        self.length = 2.
        self.level = 2
