...
class Character(object):
    ...
    def __init__(self,data, job, race, strategies, protagonist = None, generation=0, auto_loot=True):
        ...
        self.BTH_damage = 0
        self.HP_damage = 0
        self.bonus = {"HP" : 0, "BTH" : 0, "RES" : 0, "STR" : 0, "DEX" : 0, "MAG" : 0}
        ...
        self.print_action = ""
        self.action_time = 0
        self._is_shocked, self.is_shocked = 0, 0
        self._is_muted, self.is_muted = 0, 0
        self._is_dreaming, self.is_dreaming = 0, 0 
        self._is_dead, self.is_dead = 0, 0
        self.on_rythm = 0
    ...
    def BTH(self):
        ...
        return max(0, min(self.max_BTH, int(round(self._BTH +  self.bonus["BTH"] + self.race.bonus["BTH"] + self.job.bonus["BTH"] + bonus - self.BTH_damage))) )
    @BTH.setter
    def BTH(self, value):
    ...
    def HP(self):
        ...
        return max(0, min(self.max_HP, int(round(self._HP +  self.bonus["HP"] + self.race.bonus["HP"] + self.job.bonus["HP"] + bonus - self.HP_damage))) ) 
    @HP.setter
    def HP(self, value):
    ...
    def update(self,DELTATIME):
        ...
        if self.action_time <= 0:
            self.action_time = 0
            self.print_action = ""
        if self.HP <= 0 or self.max_BTH <= 0:
            self.is_dead = 1
        if self.is_dead:
            return
        ...
        self.recoil -= self.RECOIL_MULTI * DELTATIME * (1. + self.DEXmod/self.RECOIL_NORM)
        
        #BTH part
        if  not self.is_catching_breath and self.BTH <= 0:
            self.is_catching_breath = self.time_to_catch_breath()
        elif self.is_catching_breath:
            self.is_catching_breath -= DELTATIME * self.BREATH_DMG_MULTI * (1. + self.max_BTH/self.BREATH_DMG_NORM)
            if self.is_catching_breath <= 0:
                self.catchBreath()
                self.is_catching_breath = 0
        else:
            self.BTH_damage = max(0, self.BTH_damage - DELTATIME * self.BREATH_DMG_MULTI)# * (1. + self.max_BTH/self.BREATH_DMG_NORM)
        
        #HP part     
        #self.HP_damage = max(0, self.HP_damage - DELTATIME )      
        
        if self.is_dreaming:
            self.HP_damage = max(0, self.HP_damage - self.DREAM_MULTI * DELTATIME)
            self.BTH_damage = max(0, self.BTH_damage - DELTATIME * self.BREATH_DMG_MULTI * (1. + self.max_BTH/self.BREATH_DMG_NORM))
            self.recoil = self.MAX_RECOIL
                 
        #if no breath gets shocked
        if self.is_catching_breath and not self.is_shocked:
            self.is_shocked += DELTATIME
        elif self.is_shocked and not self.is_catching_breath:
        ...
        elif not self.recoil and not self.is_dreaming:
            self.take_action()  
